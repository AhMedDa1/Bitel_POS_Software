...
static int vtokeq(const char *s, jsmntok_t *t, int numtok, va_list ap) {
	if (numtok > 0) {
		...
		for (i = 0; i < numtok; i++) {
			...
			if (t[i].type != type) {
				Rprintf("token %d type is %d, not %d\n", i, t[i].type, type);
				return 0;
			}
			if (start != -1 && end != -1) {
				if (t[i].start != start) {
					Rprintf("token %d start is %d, not %d\n", i, t[i].start, start);
					return 0;
				}
				if (t[i].end != end ) {
					Rprintf("token %d end is %d, not %d\n", i, t[i].end, end);
					return 0;
				}
			}
			if (size != -1 && t[i].size != size) {
				Rprintf("token %d size is %d, not %d\n", i, t[i].size, size);
				return 0;
			}
			...
			if (s != NULL && value != NULL) {
				...
				if (strlen(value) != t[i].end - t[i].start ||
					...
					Rprintf("token %d value is %.*s, not %s\n", i, t[i].end-t[i].start,
							s+t[i].start, value);
					return 0;
				}
			}
